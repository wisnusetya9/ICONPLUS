...
class Ctrl_popap extends CI_Controller
{
	...
	public function popap_Data($page = 1)
    {
        $total = $this->Model_pop->get_total1();
        //ambil data
        $limit  =2; //menentukan limit/jumlah data yang akan ditampilkan per page
        $result = $this->Model_pop->get_all5($limit, $page);
        $url = site_url('Ctrl_popap/popap_Data/');
        $this->load->library('pagination');
        $config['base_url']         = $url;
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/list_popap', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/list_popap', $data);
		}else{
			$this->load->view('user/AP/list_popap', $data);
		}
    }
...
public function search_Data($page, $srch)
    {
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/search_pop', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/search_pop', $data);
		}else{
			$this->load->view('user/AP/search_pop', $data);
		}
    }
	...
	public function read($id_pop) 
	{
		...
		if ($row) {
			...
			$data = array(
				...
				'button' => 'Create',
				'action' => site_url('Ctrl_popap/create_action_room/'.$id_pop),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/AP/Detail_popap', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/AP/Detail_popap', $data);
			}else{
				$this->load->view('user/AP/Detail_popap', $data);
			}

		} else {
			// $this->session->set_flashdata('message', 'Recofddfrd Not Found');
			if($this->session->userdata('admin')!=''){
				redirect(site_url('admin/AP/list_popap'));
			}else if($this->session->userdata('subadmin')!=''){
				redirect(site_url('subadmin/AP/list_popap'));
			}else{
				redirect(site_url('user/AP/list_popap'));
			}
		}
	}
...
public function back()
	{
		redirect(site_url('Ctrl_popap/popap_Data/'));
	}
public function back_room($id_pop)
	{
		redirect(site_url('Ctrl_popap/read/'.$room->id));
	}
public function back_update_rack()
	{
		redirect(site_url('Ctrl_popap/createRak'));
	}
	...
	public function create() 
{
		$data = array(
			'button' => 'Create',
			'action' => site_url('Ctrl_popap/create_action/'),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/insert_popap', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/insert_popap', $data);
		}else{
			$this->load->view('user/AP/insert_popap', $data);
			}
	}
	...
	public function create_action() 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$this->upload->do_upload('Foto_POP') || !$this->upload->do_upload('Foto_POP2')) {
				...
				redirect(site_url('Ctrl_popap/create/'));
			} else {
				unset($config);
	...
	public function update($id_pop) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('Ctrl_popap/update_action/'),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/update_popap', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/update_popap', $data);
		}else{
			$this->load->view('user/AP/update_popap', $data);
			}
	...
	public function update_action($id_pop) 
    {
    	...
    	if ($this->form_validation->run() == FALSE) {
    		...
    		$this->upload->initialize($config);
    			...
    			if (!$this->upload->do_upload('Foto_POP') || !$this->upload->do_upload('Foto_POP2')) {
    				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
    				redirect(site_url('Ctrl_popap/update'));
    			} else {
    				unset($config);
    				$config['upload_path'] = 'public/POP/File';
    				$config['allowed_types'] = '*';
...
public function _rules3() 
	...
	public function createRoom($id_pop) 
	{
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/insert_room', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/insert_room', $data);
		}else{
			$this->load->view('user/AP/insert_room', $data);
			}
		}
	}
	...
	public function create_action_room($id_pop) 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$this->upload->do_upload('Foto_Room') || !$this->upload->do_upload('Foto_Room2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_popap/read/'.$id_pop));	
			} else {
				unset($config);
	...
	public function update_room($id,$id_pop) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('Ctrl_popap/update_room_action/'.$id_pop),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/update_room', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/update_room', $data);
		}else{
			$this->load->view('user/AP/update_room', $data);
			}
	...
	public function createRak($id) 
	{
		...
		if ($row) {
			...
			$data = array(
				'button' => 'Create',
				'action' => site_url('Ctrl_popap/create_action_rak/'.$id),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/AP/Detail_rack', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/AP/Detail_rack', $data);
		}else{
			$this->load->view('user/AP/Detail_rack', $data);
			}
		}
	}
	...
	public function create_action_rak($id)
	{
		...
		$this->load->library('upload');
			...
			if (!$this->upload->do_upload('Foto_Rak') || !$this->upload->do_upload('Foto_Rak2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_popap/createRak/'.$id));
			} else {
				unset($config);
	...
	public function update_rak($id_rak,$id) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'update',
				'action' => site_url('Ctrl_popap/update_rak_action/'.$id),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/AP/update_rack', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/AP/update_rack', $data);
			}else{
				$this->load->view('user/AP/update_rack', $data);
			}

			
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('Ctrl_popap/read'));
		...
		if ($row) {
			...
			$data = array(
                ...
                'perangkat' => $this->db->query("SELECT * FROM tb_perangkat where kode_rak='$rak' and Tanggal_input in(SELECT MAX(Tanggal_input) as Tanggal_input from tb_perangkat GROUP BY kode_perangkat) ")->result(),
					...
					if($this->session->userdata('admin')!=''){
						$this->load->view('admin/AP/list_perangkat', $data);
					}else if($this->session->userdata('subadmin')!=''){
						$this->load->view('subadmin/AP/list_perangkat', $data);
					}else{
						$this->load->view('user/AP/list_perangkat', $data);
					}
		}
	...
	public function createPerangkat($id) 
	{
		...
		if ($row) {
			...
			$data = array(
				'button' => 'Create',
				'action' => site_url('Ctrl_popap/create_action_perangkat/'.$id),
				'kode_room' => set_value('kode_room',$row->kode_room),
				'kode_rak' => set_value('kode_rak',$row->kode_rak),
				'Merk' => set_value('Merk'),
				'Tipe' => set_value('Tipe'),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/AP/insert_perangkat', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/AP/insert_perangkat', $data);
			}else{
				$this->load->view('user/AP/insert_perangkat', $data);
			}
	...
	public function create_action_perangkat($id)
	{
		...
		$this->load->library('upload');
			...
			if (!$this->upload->do_upload('Foto_perangkat') || !$this->upload->do_upload('Foto_perangkat2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_popap/listPerangkat/'.$id));
			} else {
				unset($config);
	...
	public function read_perangkat($id) 
	{
		...
		if ($row) {			
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/AP/Detail_perangkat', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/AP/Detail_perangkat', $data);
			}else{
				$this->load->view('user/AP/Detail_perangkat', $data);
			}
	...
	public function update_perangkat($id_perangkat,$id_rak)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'update',
				'action' => site_url('Ctrl_popap/update_perangkat_action/'.$id_rak),
				'kode_room' => set_value('kode_room',$row ->kode_room),
				'kode_rak' => set_value('kode_rak',$row ->kode_rak),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/AP/Update_perangkat', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/AP/Update_perangkat', $data);
			}else{
				$this->load->view('user/AP/Update_perangkat', $data);
				}
	...
	{
		$data = array(
		'action' => site_url('Ctrl_popap/login/'),
		);
	...
	public function login(){
		...
		// $this->load->model('Model_Login');
		if ($person == '') {
			redirect('Ctrl_popap/indexlogin', 'refresh');
		}else{
