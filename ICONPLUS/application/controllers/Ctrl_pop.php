...
class Ctrl_pop extends CI_Controller
{
	...
	public function pop_Data($page = 1)
    {
        $total = $this->Model_pop->get_total1();
        //ambil data
        $limit  =5; //menentukan limit/jumlah data yang akan ditampilkan per page
        $result = $this->Model_pop->get_all1($limit, $page);
        $url = site_url('Ctrl_pop/pop_Data/');
        $this->load->library('pagination');
        $config['base_url']         = $url;
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/list_popsb', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/list_popsb', $data);
		}else{
			$this->load->view('user/list_popsb', $data);
		}
    }
...
public function search_Data($page, $srch)
    {
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/search_pop', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/search_pop', $data);
		}else{
			$this->load->view('user/search_pop', $data);
		}
	}
  
public function detail()
{
	$this->load->view('Detail_popsb');
}
	public function read($id_pop) 
	{
		...
		if ($row) {
			...
			$data = array(
				...
				'button' => 'Create',
				'action' => site_url('Ctrl_pop/create_action_room/'.$id_pop),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/SB/Detail_popsb', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/SB/Detail_popsb', $data);
			}else{
				$this->load->view('user/Detail_popsb', $data);
			}

		} else {
			// $this->session->set_flashdata('message', 'Recofddfrd Not Found');
			if($this->session->userdata('admin')!=''){
				redirect(site_url('admin/SB/list_popsb'));
			}else if($this->session->userdata('subadmin')!=''){
				redirect(site_url('subadmin/SB/list_popsb'));
			}else{
				redirect(site_url('user/list_popsb'));
			}
		}
	}
...
public function back()
	{
		redirect(site_url('Ctrl_pop/pop_Data/'));
	}
public function back_room($id_pop)
	{
		redirect(site_url('Ctrl_pop/read/'.$room->id));
	}
public function back_update_rack()
	{
		redirect(site_url('Ctrl_pop/createRak'));
	}
	...
	public function create() 
	{
		$data = array(
			'button' => 'Create',
			'action' => site_url('Ctrl_pop/create_action/'),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/insert_popsb', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/insert_popsb', $data);
		} else {
			$this->load->view('user/insert_popsb', $data);
		}
	}
	...
	public function create_action() 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$this->upload->do_upload('Foto_POP') || !$this->upload->do_upload('Foto_POP2')) {
				...
				redirect(site_url('Ctrl_pop/create/'));
			} else {
				unset($config);
				...
				if (!$this->upload->do_upload('File_POP')) {
					...
					redirect(site_url('Ctrl_pop/pop_Data/1/'));
				} else {
	...
	public function update($id_pop) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('Ctrl_pop/update_action/'),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/update_popsb', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/update_popsb', $data);
		}else{
			$this->load->view('user/update_popsb', $data);
		}
...
public function _rules3() 
	...
	public function createRoom($id_pop) 
	{
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/insert_room', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/insert_room', $data);
		}else{
			$this->load->view('user/insert_room', $data);
		}
	  }
	}
	...
	public function create_action_room($id_pop) 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$this->upload->do_upload('Foto_Room') || !$this->upload->do_upload('Foto_Room2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_pop/read/'.$id_pop));
			} else {
				unset($config);
	...
	public function update_room($id,$id_pop) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('Ctrl_pop/update_room_action/'.$id_pop),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/update_room', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/update_room', $data);
		}else{
			$this->load->view('user/update_room', $data);
		}
	...
	public function createRak($id) 
	{
		...
		if ($row) {
			...
			$data = array(
				'button' => 'Create',
				'action' => site_url('Ctrl_pop/create_action_rak/'.$id),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/Detail_rack', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/Detail_rack', $data);
		}else{
			$this->load->view('user/Detail_rack', $data);
			}
		}
	}
	...
	public function create_action_rak($id)
	{
		...
		$this->load->library('upload');
			...
			if (!$this->upload->do_upload('Foto_Rak') || !$this->upload->do_upload('Foto_Rak2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_pop/createRak/'.$id));
			} else {
				unset($config);
	...
	public function update_rak($id_rak,$id) 
	{
		...
		if ($row) {
			$data = array(
				'button' => 'update',
				'action' => site_url('Ctrl_pop/update_rak_action/'.$id),
		...
		if($this->session->userdata('admin')!=''){
			$this->load->view('admin/SB/update_rack', $data);
		}else if($this->session->userdata('subadmin')!=''){
			$this->load->view('subadmin/SB/update_rack', $data);
		}else{
		...
		if ($row) {
			...
			$data = array(
                ...
                'perangkat' => $this->db->query("SELECT * FROM tb_perangkat where kode_rak='$rak' and Tanggal_input in(SELECT MAX(Tanggal_input) as Tanggal_input from tb_perangkat GROUP BY kode_perangkat) ")->result(),
					...
					if($this->session->userdata('admin')!=''){
					$this->load->view('admin/SB/list_perangkat', $data);
				}else if($this->session->userdata('subadmin')!=''){
					$this->load->view('subadmin/SB/list_perangkat', $data);
				}else{
					$this->load->view('user/list_perangkat', $data);
				}
			}
		}
	...
	public function createPerangkat($id) 
	{
		...
		if ($row) {
			...
			$data = array(
				'button' => 'Create',
				'action' => site_url('Ctrl_pop/create_action_perangkat/'.$id),
				'kode_room' => set_value('kode_room',$row->kode_room),
				'kode_rak' => set_value('kode_rak',$row->kode_rak),
				'Merk' => set_value('Merk'),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/SB/insert_perangkat', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/SB/insert_perangkat', $data);
			}else{
				$this->load->view('user/insert_perangkat', $data);
			}			
		}
	}
	...
	public function create_action_perangkat($id)
	{
		...
		$this->load->library('upload');
			...
			if (!$this->upload->do_upload('Foto_perangkat') || !$this->upload->do_upload('Foto_perangkat2')) {
				$this->session->set_flashdata('message', 'Type Data tidak diizinkann harus jpg|png');
				redirect(site_url('Ctrl_pop/listPerangkat/'.$id));
			} else {
				unset($config);
	...
	public function read_perangkat($id) 
	{
		...
		if ($row) {			
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/SB/Detail_perangkat', $data);
			}else if($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/SB/Detail_perangkat', $data);
			}else{
				$this->load->view('user/Detail_perangkat', $data);
			}
	...
	public function update_perangkat($id_perangkat,$id)
	{
		$row = $this->Model_perangkat->get_by_id2($id_perangkat);
		...
		if ($row) {
			$data = array(
				'button' => 'update',
				'action' => site_url('Ctrl_pop/update_perangkat_action/'.$id),
				'kode_room' => set_value('kode_room',$row ->kode_room),
				'kode_rak' => set_value('kode_rak',$row ->kode_rak),
			...
			if($this->session->userdata('admin')!=''){
				$this->load->view('admin/SB/Update_perangkat', $data);
			}else if ($this->session->userdata('subadmin')!=''){
				$this->load->view('subadmin/SB/Update_perangkat', $data);
			}else {
				$this->load->view('user/Update_perangkat', $data);
			}
		}
	}
	...
	{
		$data = array(
		'action' => site_url('Ctrl_pop/login/'),
		);
		$this->load->view('admin/SB/login_view',$data);
	}
	...
	public function login(){
		...
		// $this->load->model('Model_Login');
		if ($person == '') {
			redirect('Ctrl_pop/indexlogin', 'refresh');
		}else{
